...
namespace Snake
{
    ...
    bool Controller::IsOverTheMap(Segment &newHead)
    {
        return newHead.x < 0 or newHead.y < 0 or newHead.x >= m_mapDimension.first or newHead.y >= m_mapDimension.second;
    }

    bool Controller::IsFoodFound(Segment &newHead)
    {
        return std::make_pair(newHead.x, newHead.y) == m_foodPosition;
    }

    bool Controller::SnakeHitsHimself(Segment &newHead)
    {
        bool lost = false;

        for (auto segment : m_segments)
        { // change names m_segments
            if (segment.x == newHead.x and segment.y == newHead.y)
            {                                                           // are segments same position
    ...
    void Controller::TimeOutHandler(const TimeoutInd &)
    {
        Segment const &currentHead = m_segments.front();
        ...
        if (SnakeHitsHimself(newHead))
            return;

        if (IsFoodFound(newHead))
        {
            m_scorePort.send(std::make_unique<EventT<ScoreInd>>());
            m_foodPort.send(std::make_unique<EventT<FoodReq>>());
        }
        else if (IsOverTheMap(newHead))
        {
